...
remote_latest=$(curl 'https://api.github.com/repos/deta/surf/releases/latest' -s)
...
update_version() {
  ...
  if [ "$arch" = "aarch64" ]; then
    appimage_download_url="https://github.com/deta/surf/releases/download/$remote/Surf-$remote.arm64.AppImage"
  else
    appimage_download_url="https://github.com/deta/surf/releases/download/$remote/Surf-$remote.x86_64.AppImage"
  fi
...
main() {
  ...
  if ! git diff --exit-code >/dev/null; then
    ...
    message="$message surf @ $commit_targets to $commit_version"
